...
export const ATTESTATION_BADGE_STYLE_OPTIONS: Array<{
...
}> = [
  {
    value: "ledger-strip",
    label: "Ledger Strip",
    description: "Wide metadata-first badge with file, runtime, scan date, and short hash.",
  },
  {
    value: "split-pill",
    label: "Split Pill",
    description: "Compact two-segment badge focused on the brand, runtime, and verdict.",
  },
  {
...
function badgeToneFromPayload(payload: PublicAttestationPayload): AttestationBadgeTone {
...
function badgeBrandLabelFromPayload(payload: PublicAttestationPayload): string {
  return payload.verificationTier === "source_verified"
    ? "MLVScan verified"
    : "MLVScan attested"
}
...
export function resolveAttestationBadgeMetadata(
  ...
  if (payload.badge) {
    return {
      ...payload.badge,
      style: normalizeAttestationBadgeStyle(payload.badge.style),
    }
  }

  return {
    schemaVersion: "badge.v1",
    style: normalizeAttestationBadgeStyle(payload.badgeStyle),
    brand: {
      kind: "mlvscan-check",
    ...
    fileLabel: badgeFileLabel(payload.fileName),
    verificationLabel:
...
function badgeClassicColor(tone: AttestationBadgeTone): string {
  switch (tone) {
    case "revoked":
      return "#52606d"
    case "known-threat":
...
function badgeShieldsColor(tone: AttestationBadgeTone): string {
  switch (tone) {
    case "revoked":
      return "#9f9f9f"
    case "known-threat":
...
function badgeFlatColor(tone: AttestationBadgeTone): string {
  switch (tone) {
    case "revoked":
      return "#6e7781"
    case "known-threat":
...
function buildCompactMetadataParts(
  metadata: PublicAttestationBadgeMetadata,
): string[] {
  return [
    metadata.runtimeLabel,
    metadata.fileLabel,
    metadata.scannedDateLabel,
    metadata.shortHashLabel,
  ].filter((value): value is string => Boolean(value))
}

function buildSignatureMetadataParts(
  metadata: PublicAttestationBadgeMetadata,
): string[] {
  return [
    metadata.fileLabel,
...
function renderLedgerStripBadgeSvg(metadata: PublicAttestationBadgeMetadata): string {
  const descriptor = buildCompactMetadataParts(metadata).join(" | ")
  const color = badgeClassicColor(metadata.tone)
  const label = "MLVScan"
  const leftWidth = Math.max(68, label.length * 7 + 18)
  const rightWidth = Math.max(152, descriptor.length * 7 + 18)
  const width = leftWidth + rightWidth
  const ariaLabel = `${label}: ${metadata.statusLabel}; ${descriptor}`

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="28" role="img" aria-label="${escapeSvgText(ariaLabel)}"><linearGradient id="g" x2="0" y2="100%"><stop offset="0" stop-color="#fff" stop-opacity=".08"/><stop offset="1" stop-opacity=".08"/></linearGradient><clipPath id="r"><rect width="${width}" height="28" rx="6" fill="#fff"/></clipPath><g clip-path="url(#r)"><rect width="${leftWidth}" height="28" fill="#0f1f29"/><rect x="${leftWidth}" width="${rightWidth}" height="28" fill="${color}"/><rect width="${width}" height="28" fill="url(#g)"/></g><g fill="#fff" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11"><text x="${leftWidth / 2}" y="19">${escapeSvgText(label)}</text><text x="${leftWidth + (rightWidth / 2)}" y="19">${escapeSvgText(descriptor)}</text></g></svg>`
}

function renderSplitPillBadgeSvg(
  payload: PublicAttestationPayload,
  metadata: PublicAttestationBadgeMetadata,
): string {
  const brandLabel = metadata.runtimeLabel
    ? `${metadata.brand.label} / ${metadata.runtimeLabel}`
    : metadata.brand.label
  const title = `${payload.fileName.trim() || payload.publicDisplayName.trim() || "Unnamed file"} - ${brandLabel} - ${metadata.statusLabel.toLowerCase()} - scanned ${metadata.scannedDateLabel} - sha256 ${metadata.shortHashLabel}`
  const segmentIds = badgeSegmentIds(payload)
  const labelWidth = Math.max(82, Math.ceil(brandLabel.length * 6.7 + 20))
  const messageWidth = Math.max(58, Math.ceil(metadata.statusLabel.length * 6.7 + 20))
  const width = labelWidth + messageWidth
  const color = badgeFlatColor(metadata.tone)

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="20" role="img" aria-label="${escapeSvgText(`${brandLabel}: ${metadata.statusLabel}`)}"><title>${escapeSvgText(title)}</title><linearGradient id="${segmentIds.gradientId}" x2="0" y2="100%"><stop offset="0" stop-color="#fff" stop-opacity=".1"/><stop offset="1" stop-opacity=".1"/></linearGradient><clipPath id="${segmentIds.clipId}"><rect width="${width}" height="20" rx="4" fill="#fff"/></clipPath><g clip-path="url(#${segmentIds.clipId})"><rect width="${labelWidth}" height="20" fill="#555"/><rect x="${labelWidth}" width="${messageWidth}" height="20" fill="${color}"/><rect width="${width}" height="20" fill="url(#${segmentIds.gradientId})"/></g><g fill="#fff" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11"><text x="${labelWidth / 2}" y="15">${escapeSvgText(brandLabel)}</text><text x="${labelWidth + (messageWidth / 2)}" y="15">${escapeSvgText(metadata.statusLabel)}</text></g></svg>`
}

function renderClassicShieldBadgeSvg(metadata: PublicAttestationBadgeMetadata): string {
  const descriptor = buildCompactMetadataParts(metadata).join(" | ")
  const color = badgeShieldsColor(metadata.tone)
  const label = "MLVScan"
...
function renderSignatureBarBadgeSvg(
  ...
  return [
    ...
    `  </g>`,
    `  <text x="${brandWidth + statusWidth / 2}" y="19" text-anchor="middle" font-family="Inter, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif" font-size="11" font-weight="700" fill="${statusColor.text}">${escapeSvgText(metadata.statusLabel)}</text>`,
    `  <text x="${brandWidth + statusWidth + 10}" y="19" font-family="Inter, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif" font-size="10" fill="#9cb0ba">${escapeSvgText(metadataRail)}</text>`,
    `</svg>`,
  ].join("\n")
}
...
export function renderAttestationBadgeSvg(
  ...
  const badgeStyle = badgeStyleOverride ?? normalizeAttestationBadgeStyle(metadata.style)
  ...
  switch (badgeStyle) {
    case "split-pill":
      return renderSplitPillBadgeSvg(payload, effectiveMetadata)
    case "classic-shield":
      return renderClassicShieldBadgeSvg(effectiveMetadata)
    case "signature-bar":
      return renderSignatureBarBadgeSvg(effectiveMetadata)
    case "ledger-strip":
    default:
      return renderLedgerStripBadgeSvg(effectiveMetadata)
  }
}
