...
export default function WasmInteropContent() {
  return (
    <>
      ...
      <p className="text-gray-300 mb-6">
        Result payloads are serialized against the shared Core-owned schema, not a WASM-specific hand-maintained shape. That keeps interop output aligned with the CLI, API, and desktop consumers.
      </p>
