...
export default function WasmInteropContent() {
  return (
    <>
      ...
      <CodeBlock language="csharp" className="mb-6">{`using System.Runtime.InteropServices.JavaScript;
...
public static partial class ScannerExports
{
    ...
    [JSExport]
    public static string GetVersion() => MLVScanVersions.CoreVersion;

    [JSExport]
    public static string GetSchemaVersion() => MLVScanVersions.SchemaVersion;
}`}</CodeBlock>
      ...
      <CodeBlock language="typescript" className="mb-6">{`// Retrieve exports from the initialized runtime
...
const result = wasmExports.ScanAssembly(fileBytes, "MyMod.dll");`}</CodeBlock>
