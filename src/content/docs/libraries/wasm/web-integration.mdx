...
export default function WasmIntegrationContent() {
  return (
    <>
      ...
      <CodeBlock language="typescript" className="mb-6">{`import { initScanner, getScannerStatus } from '@mlvscan/wasm-core'
...
if (status.isMock) {
  console.info('Running in mock mode', status.mockRequestedExplicitly ? '(requested)' : '(fallback)')
      ...
      <CodeBlock language="typescript" className="mb-6">{`import { scanAssembly } from '@mlvscan/wasm-core'
...
export async function scanFile(file: File) {
  ...
  return result
      ...
      <CodeBlock language="typescript" className="mb-6">{`import { isScannerReady, isMockScanner, getInitError, getScannerVersion } from '@mlvscan/wasm-core'
