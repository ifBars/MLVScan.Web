  ...
  return (
    <>
      ...
      <p className="text-gray-300 mb-4">
        ...
        method. In the historical scanner build that produced this advisory, when both <code>HasNetworkCall</code> and <code>HasFileWrite</code> are set in the same
...
if (HasNetworkCall && HasFileWrite)
    return true;`}
      ...
      <p className="text-gray-300 mb-4">
        This combination was treated as a possible payload dropper because malicious droppers often
        download data and write it to disk before execution. In this case, the finding lacked the
        context to distinguish between a dropper writing an executable and a mod writing a PNG to its own
        UserData folder.
      </p>
...
private IEnumerator EnsureIconFileExists() {
    ...
    if (!File.Exists(iconPath)) {
      ...
      <p className="text-gray-400 text-sm mb-4">
        Version context: this was a historical false positive. Current MLVScan.Core builds do not classify
        a plain network download plus file write as the <code>DownloadAndExecute</code> family on its own; that
        correlated pattern requires an execution sink such as <code>Process.Start</code>. The retained BankApp
        sample is covered by false-positive regression tests and should scan cleanly on current builds.
      </p>
      <ul className="list-disc pl-6 space-y-2 text-gray-300 mb-6">
      ...
      <p className="text-gray-300 mb-6">
        The historical <code>IsHighRiskCombination()</code> check in <code>MethodSignals</code> treated the
      ...
      <p className="text-gray-300 mb-4">
        ...
        simple <code>Assembly.GetManifestResourceStream</code> call. BankApp 2.0.4 and later produced
        zero findings in the affected scanner line, and the retained BankApp sample remains part of the
        current false-positive regression suite.
      </p>
